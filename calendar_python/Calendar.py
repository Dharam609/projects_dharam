...
def command_add(date, start_time, end_time, title, calendar):
    ...
    if start_time <= end_time:
        if date in calendar:
            another_event = [{'start':start_time, 'end':end_time, 'title':title}]
            calendar[date].append(another_event)
            return True
        elif date not in calendar and is_calendar_date(date):
...
def is_calendar_date(date):
    ...
    if len(date) != 10:
        return False
    # "-" on 4th and 7th index makes sure of the length of year(4), month(2) and day(2)
    if date[4] == date[7] != "-":
        return False
    else:
        ...
        if len(date_parts) == 3:
            ...
            month = date_parts[1]
            day = date_parts[2]
            if is_natural_number(year) and is_natural_number(month) and is_natural_number(day):
                if (int(year) > 0) and (0 < int(month) < 13) and (0 < int(day) < 32):
                    return True
                else:
                    return False
            else:
...
def is_natural_number(str):
    ...
    match_string = "0123456789"
    if str:
        for i in str:
            if i not in match_string:
                return False
    else:
        return False
    return True
